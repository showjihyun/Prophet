...
from sqlalchemy import String, Integer, Float, Boolean, SmallInteger, Text, DateTime, func, ARRAY
...
class ExpertOpinion(Base):
    ...
    simulation_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False)
    expert_agent_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False)
    step: Mapped[int] = mapped_column(Integer, nullable=False)
...
class EmergentEvent(Base):
    ...
    community_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True))
